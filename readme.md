...
tcpdump -nn -i eth0 src host <IP сервера>           # пакеты, отправленные с этого IP
tcpdump -nn -i eth0 dst host <IP сервера>           # пакеты, отправленыые на этот IP
tcpdump -nn -i eth0 src net 172.17.254.0/24         # пакеты, приходящие  из указанной подсети
tcpdump -nn -i eth0 dst net 172.17.254.0/24         # пакеты, направленные в указанную подсеть
