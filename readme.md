...
top                    # мониторинг процессов, -i только активные, -u root только от пользователя root           
...
df -i                   # проверка inode
du -ah                  # итоговый размер по папкам и вложенным директориям
du -h                   # итоговый размер по папкам
du -h --max-depth=0     # размер только текущих каталогов (без подкаталогов)
du -sh *                # итоговый размер по папкам (показывает итоговый размер)
...
awk '{print $1, $3}' file.txt               # вывести первую и третью колонку
awk '$6 == "404" {print $1, $2}' file.txt   # если шестая колонка равна 404, то вывести первую и вторую колонки 
sed 's/old/new/g' file.txt                  # заменить текст
cut -d ':' -f 2 /etc/passwd                 # извлечь вторую колонку после разделителя, ":" - разделитель между колонками
...
sort -hr                                    # сортивка в human, рекурсивно
sort -t: -k2 -n file.txt                    # сортировать вторую колонку по числу, после разделителя ":"
sort -n file.txt                            # сортировать по числу, а не по алфавиту 
...
- n — сортировать по имени (Name)
- s — сортировать по размеру (Size)
- C — сортировать по количеству элементов (Count)
- M — сортировать по времени изменения (Modified time)
